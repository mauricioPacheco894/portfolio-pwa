...
export function useTransaction({ portfolioId, onSuccess }: UseTransactionOptions) {
    ...
    const createTransaction = async (data: Omit<TransactionFormData, 'id' | 'portfolio_id'>) => {
        if (!user) {
    ...
    const deleteTransaction = async (id: string) => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : 'Error al eliminar transacción';
            setError(message);
            toast.error(message);
            return { success: false, error: message };
        } finally {
